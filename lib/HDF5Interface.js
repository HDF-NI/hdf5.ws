...
HDF5Interface.prototype.setCompression = function(path){
    ...
    path=encodeURIComponent(path);
    xmlHttp.open( "POST", "http://"+location.hostname+(location.port ? ":"+location.port: "")+"/set_compression/"+path ); // false for synchronous request
    xmlHttp.send( );
}
...
HDF5Interface.prototype.makeDataset = function(path, data, cb){
    ...
    ws.addEventListener('open', function open() {
      ...
      ws.send(data.buffer, {compress: false});
      //ws.end();
      ws.close();
    });
...
HDF5Interface.prototype.readText = function(path, read){
    ...
    var xmlHttp = new XMLHttpRequest();
    xmlHttp.addEventListener("load", function(evt){
        console.log("XMLHttpRequest load "+location.hostname);
...
HDF5Interface.prototype.getImageInfo = function(path){
    ...
    path=encodeURIComponent(path);
    xmlHttp.open( "POST", "http://"+location.hostname+(location.port ? ":"+location.port: "")+"/get_image_info/"+path, false ); // false for synchronous request
    xmlHttp.send( );
    return metaData;
}
...
HDF5Interface.prototype.requestImage = function(path, read){
    ...
    var xmlHttp = new XMLHttpRequest();
    xmlHttp.addEventListener("load", function(evt){
        console.log("XMLHttpRequest load read image");
...
HDF5Interface.prototype.requestImageRegion = function(path, metaDataInput, read){
    ...
    xmlHttp.addEventListener("load", function(evt){
        console.log("XMLHttpRequest load read image");
        var ws = new WebSocket('ws://'+location.hostname+':'+_this.port+'/read-image-region');
        ws.binaryType = 'arraybuffer';
        ws.addEventListener('open', function open() {
    ...
    path=encodeURIComponent(path);
    xmlHttp.open( "POST", "http://"+location.hostname+(location.port ? ":"+location.port: "")+"/read_image_region/"+path ); // false for synchronous request
    xmlHttp.send( );
}
...
HDF5Interface.prototype.requestImageMosaic = function(path, metaDataInput, read){
    ...
    xmlHttp.addEventListener("load", function(evt){
        console.log("XMLHttpRequest load read image");
        var ws = new WebSocket('ws://'+location.hostname+':'+_this.port+'/read-image-mosaic');
        ws.binaryType = 'arraybuffer';
        ws.addEventListener('open', function open() {
    console.log(JSON.stringify(metaDataInput));
    ws.send(JSON.stringify(metaDataInput));
    ...
    path=encodeURIComponent(path);
    xmlHttp.open( "POST", "http://"+location.hostname+(location.port ? ":"+location.port: "")+"/read_image_mosaic/"+path ); // false for synchronous request
    xmlHttp.send( );
    //});
}
