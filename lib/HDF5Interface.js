...
HDF5Interface.prototype.makeImage = function(path, data, metaData, cb){
    ...
    xmlHttp.addEventListener("load", function(evt){
        console.log("XMLHttpRequest load");
    var ws = new WebSocket('ws://'+location.hostname+':9900/make-image');
    ...
    path=encodeURIComponent(path);
    xmlHttp.open( "POST", "http://"+location.hostname+":8888/make_image/"+path ); // false for synchronous request
    xmlHttp.send( );
...
HDF5Interface.prototype.readImage = function(path, read){
    ...
    xmlHttp.addEventListener("load", function(evt){
        console.log("XMLHttpRequest load");
        var ws = new WebSocket('ws://'+location.hostname+':9900/read-image');
        ws.binaryType = 'arraybuffer';
        ws.addEventListener('open', function open() {
    ...
    path=encodeURIComponent(path);
    xmlHttp.open( "POST", "http://"+location.hostname+":8888/read_image/"+path ); // false for synchronous request
    xmlHttp.send( );
